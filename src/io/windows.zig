...
pub const IO = struct {
    ...
    fn open_file_handle(relative_path: []const u8, method: enum { create, open }) !std.posix.fd_t {
        const path_w = try os.windows.sliceToPrefixedFileW(std.fs.cwd().fd, relative_path);
